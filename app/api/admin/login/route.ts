...
import { ADMIN_COOKIE, ADMIN_PASSWORD, makeSessionToken, verifyAdminPassword } from "@/lib/auth";

/** 관리자 로그인 — 환경변수 ADMIN_PASSWORD 검증 후 HMAC 쿠키 발급 */
export async function POST(req: Request) {
  const { password } = await req.json().catch(() => ({}));
  if (!password || password !== ADMIN_PASSWORD && !(await verifyAdminPassword(password)))
